...
/// </summary>
public partial class DocumentManagementPage : Page, INavigableView<DocumentManagementViewModel>
{
	...
	private void UpdateButtonsVisibility()
	{
		...
		if (selectedTreeNode == null)
		{
			...
			BtnOpenMd.Visibility = Visibility.Collapsed;
		}
		...
		else if (selectedTreeNode.File != null)
		{
			...
			BtnOpenMd.Visibility = ((!(text == "markdown")) ? Visibility.Collapsed : Visibility.Visible);
		}
	}
	...
	private void RenderPdfPreview(DocFile file)
	{
		...
		// 对 Base64 编码后的字符串进行 URL 编码，避免特殊字符（+、/、=）在查询参数中被错误解析
		string encodedPath = Uri.EscapeDataString(value2);
		string text = $"{value}/FileService/DownloadFile?RelativePath={encodedPath}&ApiKey={ViewModel.storage.Config.Key}";
		...
		return Regex.Replace(markdown, pattern, delegate(Match match)
		{
			...
			if (!value2.StartsWith("/"))
			{
				try
				{
					...
					// 对 Base64 编码后的字符串进行 URL 编码，避免特殊字符在查询参数中被错误解析
					string encodedPath = Uri.EscapeDataString(value4);
					string value5 = $"{value3}/FileService/DownloadFile?RelativePath={encodedPath}&ApiKey={ViewModel.storage.Config.Key}";
					return $"![{value}]({value5})";
				}
				catch (Exception)
				{
	...
	/// 打开md
	/// </summary>
	private async void OpenMd_Click(object sender, RoutedEventArgs e)
	{
		DocumentTreeNode selectedTreeNode = ViewModel.SelectedTreeNode;
		if (selectedTreeNode?.File == null)
		{
			await ViewModel.message.WarnAsync("请先选择要打开的文件！");
			return;
		}
		
		await ViewModel.OpenMdFileAsync(selectedTreeNode.File);
	}


    // 新增：打开文档管理为独立窗口，与其他菜单同时使用（独立状态）
    private async void OpenStandaloneWindow_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            var newVm = new DocumentManagementViewModel(ViewModel.message, ViewModel.picker, ViewModel.context, ViewModel.storage, ViewModel.model);
            await newVm.LoadTreeAsync();
            ...
            var page = new DocumentManagementPage(newVm);
            window.Content = page;
            window.Show();
        }
        catch (Exception)
